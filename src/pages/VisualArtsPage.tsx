...
import { visualArtsData } from '../data/galleryData';
...
export default function VisualArtsPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div
        ref={panelRef}
        className="w-full lg:w-[55%] lg:fixed lg:right-0 lg:top-0 lg:h-screen overflow-y-auto"
        style={{
        ...
        <div ref={contentRef} className="px-8 md:px-12 lg:px-16 py-20 lg:py-28 max-w-2xl">
          ...
          {data.featuredNote && (
            <div className="reveal-item mb-10">
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '15px', lineHeight: '1.7em', color: 'var(--text-primary)' }}>
                <strong>{data.featuredNote.split(data.featuredLinkText || '')[0]}</strong>
                {data.featuredLink && (
                  <a href={data.featuredLink} target="_blank" rel="noopener noreferrer" className="underline hover:text-[var(--accent-coral)]" style={{ color: 'var(--accent-coral)' }}>
                    {data.featuredLinkText}
                  </a>
                )}
              </p>
            </div>
          ...
          {/* Gallery Grid */}
          <div ref={galleryRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-10">
